...
public class MarkExpeditionInTransitHandlerTests
{
    [Fact]
    public async Task Handle_ShouldAdvanceStatus_AndPublishEvent()
    {
        ...
        var handler = new MarkExpeditionInTransitHandler(repository, publisher);

        var result = await handler.Handle(new MarkExpeditionInTransitCommand(expedition.OrderId), CancellationToken.None);

        Assert.Equal("InTransit", result.Status);
        Assert.Equal(1, repository.SaveChangesCalls);
        Assert.Single(publisher.PublishedStatuses);
        Assert.Equal("InTransit", publisher.PublishedStatuses.Single());
    }

    [Fact]
    public async Task Handle_ShouldThrow_WhenOrderDoesNotExist()
    {
        var handler = new MarkExpeditionInTransitHandler(new FakeExpeditionRepository(), new FakeExpeditionEventPublisher());

        await Assert.ThrowsAsync<KeyNotFoundException>(() =>
            handler.Handle(new MarkExpeditionInTransitCommand(Guid.NewGuid()), CancellationToken.None));
    }
