...
public class MarkExpeditionPickedUpHandlerTests
{
    [Fact]
    public async Task Handle_ShouldAdvanceStatus_AndPublishEvent()
    {
        ...
        var handler = new MarkExpeditionPickedUpHandler(repository, publisher);

        var result = await handler.Handle(new MarkExpeditionPickedUpCommand(expedition.OrderId), CancellationToken.None);

        Assert.Equal("PickedUpByCarrier", result.Status);
        Assert.Equal(1, repository.SaveChangesCalls);
        Assert.Single(publisher.PublishedStatuses);
        Assert.Equal("PickedUpByCarrier", publisher.PublishedStatuses.Single());
    }

    [Fact]
    public async Task Handle_ShouldThrow_WhenOrderDoesNotExist()
    {
        var handler = new MarkExpeditionPickedUpHandler(new FakeExpeditionRepository(), new FakeExpeditionEventPublisher());

        await Assert.ThrowsAsync<KeyNotFoundException>(() =>
            handler.Handle(new MarkExpeditionPickedUpCommand(Guid.NewGuid()), CancellationToken.None));
    }
